...
class TempModel {
  ...
  TempModel.fromJson(Map<String, dynamic> json) {
    ...
    addons = json.containsKey('addons') ? List<TempAddons>.from((json['addons'] as List<dynamic>).map((e) => TempAddons.fromJson(e))).toList() : [];
    qty = json['qty'] ?? 0;
...
class TempAddons {
  ...
  TempAddons({this.id, this.name, this.price, this.isActive, this.image, this.qty = 0});

  TempAddons.fromJson(Map<String, dynamic> json) {
    id = json['id'];
    isActive = json['isActive'];
    name = json['name'];
    price = json['price'] ?? '0';
