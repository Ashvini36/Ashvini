...
class CustomersController extends GetxController {
    ...
    try {
      if (!email.value.text.isEmail) {
        ...
        UserModel model = UserModel(
            id: userId,
            branchId: selectBranchCust.value.id ?? Constant.selectedBranch.id,
  ...
  searchByName({String name = ''}) async {
    ...
    FireStoreUtils.getAllUserByName(name, Constant.customerRole).then((value) {
      customerList.value = value ?? [];
      setPagition(totalItemPerPage.value);
    });
  }
  ...
  Rx<BranchModel> selectBranchCust = BranchModel().obs;
  setDefaultData() {
    ...
    if(Constant.allBranch.isNotEmpty){
      selectBranchCust.value = selectBranchCust.value = Constant.allBranch.where((element) => element.id == Constant.selectedBranch.id).first;
    }
    status.value = 'Active';
  }
  ...
  editCustomerData(UserModel model) {
    ...
    profileImageUin8List.value = Uint8List(100);
    selectBranchCust.value = Constant.allBranch.where((element) => element.id == model.branchId).first;
    status.value = 'Active';
  }
