...
class PosTableController extends GetxController {
  RxString title = "Pos Tables".obs;
  ...
  getData() async {
    ...
    await getDiningTableData();
    await getItemCategoriesData();
    await getAllItem();
    await getItemAttributeData();
  }

  RxList<DiningTableModel> diningTableList = <DiningTableModel>[].obs;
  RxBool isTableLoading = true.obs;
  RxString selectTableId = ''.obs;

  Future<void> getDiningTableData({String name = ''}) async {
    await FireStoreUtils.getDiningTable(name).then((tablelist) {
      isTableLoading.value = false;
      if (tablelist?.isNotEmpty == true) {
        diningTableList.value = tablelist!;
      }
      update();
    });
  }
  ...
  Future<void> getOrderByTable({required String tableid}) async {
    await FireStoreUtils.getOrderByTableId(tableId: tableid).then((value) {
  ...
  orderPlace() async {
    ...
    if (addToCart.value.id != null) {
      order = OrderModel(
          id: addToCart.value.id,
          paymentStatus: addToCart.value.paymentStatus,
          tableId: addToCart.value.tableId,
          paymentMethod: addToCart.value.paymentMethod,
          ...
          taxList: addToCart.value.taxList,
          branchId: addToCart.value.branchId,
          status: addToCart.value.status,
          createdAt: addToCart.value.createdAt ?? Timestamp.now(),
          updatedAt: Timestamp.now(),
          type: Constant.orderTypeCustomer);
    } else {
      order = OrderModel(
          paymentStatus: false,
          tableId: selectTableId.value,
    ...
    await FireStoreUtils.orderPlace(order).then((value) async {
      await FireStoreUtils.getAllUserByBranch(order.branchId.toString()).then((value) async {
        if (value != null) {
          for (var element in value) {
            if (element.notificationReceive == true) {
              if (element.fcmToken != null && element.fcmToken?.isNotEmpty == true) {
      ...
      update();
      Get.back();
      ShowToastDialog.showToast("Order Save successfully!");
    });
  }
