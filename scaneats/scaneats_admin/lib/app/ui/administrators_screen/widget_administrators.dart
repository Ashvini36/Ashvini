...
class AllAdministratorsWidget extends StatelessWidget {
  const AllAdministratorsWidget({super.key});
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Row(mainAxisAlignment: Responsive.isMobile(context) ? MainAxisAlignment.start : MainAxisAlignment.end, children: [
                                ...
                                spaceW(),
                                Constant.selectedRole.permission!.firstWhere((element) => element.title == "Administrators").isUpdate == false
                                    ? const SizedBox()
                                    : RoundedButtonFill(
                                        isRight: true,
                                        ...
                                        onPress: () {
                                          if (int.parse(Constant.restaurantModel.subscription!.noOfAdmin.toString()) > controller.customerList.length) {
                                ...
                                : DataTable(
                                    ...
                                    columns: [
                                      ...
                                      const DataColumn(
                                        ...
                                        child: Row(
                                          children: [
                                            TextCustom(title: 'Phone Number'),
                                          ],
                                        ),
                                      )),
                                      DataColumn(
                                          label: SizedBox(
                                        width: Responsive.isMobile(context) ? 100 : MediaQuery.of(context).size.width * 0.15,
                                        child: const Row(
                                          children: [
                                            TextCustom(title: 'Status'),
                                          ],
                                        ),
                                      )),
                                      DataColumn(
                                          label: SizedBox(
                                        width: Responsive.isMobile(context) ? 100 : MediaQuery.of(context).size.width * 0.14,
                                        child: const Row(
                                          children: [
                                            TextCustom(title: 'Actions'),
                                          ],
                                        ),
                                      ))
                                    ],
                                    rows: controller.currentPageUser
                                        .map((e) => DataRow(cells: [
                                              ...
                                              DataCell(Row(children: [
                                                ...
                                                spaceW(),
                                                Constant.selectedRole.permission!.firstWhere((element) => element.title == "Administrators").isDelete == false
                                                    ? const SizedBox()
                                                    : IconButton(
                                                        onPressed: () {
                                                          if (e.restaurantId == CollectionName.restaurantId) {
                                                            ShowToastDialog.showToast("This User Is Not able to delete", duration: Constant.toastDuration);
                                                          } else {
                                                            if (Constant.isDemo()) {
...
class FilterPopUp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final themeChange = Provider.of<DarkThemeProvider>(context);
    return GetX<AdministratorsController>(
        init: AdministratorsController(),
        builder: (controller) {
...
WidgetBuilder horizontalDrawerBuilder({bool isEdit = false, bool isSave = true}) {
  return (BuildContext context) {
    ...
    return GetX<AdministratorsController>(
        ...
        builder: (controller) {
          return Drawer(
            backgroundColor: themeChange.getThem() ? AppThemeData.pickledBluewood950 : AppThemeData.white,
            width: 500,
            child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                                ...
                                onTap: () {
                                  showDialog(context: context, builder: (ctxt) => const PickUpImageWidget()).then((value) async {
                                    if (value != null) {
                                      ...
                                      printLog("Image :: ${stringFile.path}");
                          ...
                          Row(children: [
                            Flexible(
                              child: customRadioButton(context,
                                  title: 'BRANCH', radioOne: "Current Branch", parameter: controller.selectedBranchRadio.value, radioTwo: "All Branch", onChangeOne: () {
                                if (isSave) {
                                  controller.selectedBranchRadio.value = "Current Branch";
                                  controller.update();
                                }
                              }, onChangeTwo: () {
                                if (isSave) {
                                  controller.selectedBranchRadio.value = "All Branch";
                                  controller.update();
                                }
                              }),
                            ),
