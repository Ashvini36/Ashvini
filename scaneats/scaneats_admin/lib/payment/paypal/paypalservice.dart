...
class FlutterPaypalSDK {
  ...
  void handlePayPalPayment(BuildContext context, {Payment? payment}) async {
    if (kIsWeb) {
      html.window.open(payment?.approvalUrl ?? '', "_self");
    } else {
      Navigator.push(context, MaterialPageRoute(builder: (context) => WebViewScreen(initialURl: payment?.approvalUrl ?? '')));
    }
  }
