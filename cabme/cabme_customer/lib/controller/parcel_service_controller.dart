...
class ParcelServiceController extends GetxController {
  ...
  Future<dynamic> getParcel() async {
    try {
      final response = await http.get(Uri.parse("${API.getParcel}?id_user_app=${Preferences.getInt(Preferences.userId)}"), headers: API.header);
      Map<String, dynamic> responseBody = json.decode(response.body);
  ...
  bookParcelRide() async {
    try {
      ...
      request.fields['receiver_phone'] = rPhoneController.text.toString().trim();
